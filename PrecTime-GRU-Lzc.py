...
class PrecTime(nn.Module):
    def __init__(
        ...
        self.gru1_dimension = gru1_dimension
        self.gru2_dimension = gru2_dimension
        ...
        # 中间LSTM层
        self.context_detection1 = nn.GRU(
            input_size=self.fe_fc_dimension,
            hidden_size=self.gru1_dimension,
            num_layers=1,
            bidirectional=True,
            batch_first=True
        )
        self.context_detection2 = nn.GRU(
            input_size=self.gru1_dimension * 2,
            hidden_size=self.gru2_dimension,
            num_layers=1,
            bidirectional=True,
            batch_first=True
        )
        ...
        self.inter_fc = nn.Linear(
            in_features=self.gru2_dimension * 2,
        ...
        self.prediction_refinement = nn.Sequential(
            conv1d_block(
                in_channels=self.hidden_channels * 2 + self.gru2_dimension * 2,
    ...
    def forward(self, x):
        ...
        print("The output shape after first GRU:", context1.shape)
        context2, _ = self.context_detection2(context1)
        print("The output shape after second GRU:", context2.shape)
